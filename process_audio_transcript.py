...
def process_audio_transcript(transcription):
    ...
    for i in range(len(transcription.words) - 1):
        ...
        if current_word['word'].lower() in filler_words:
            filler_word_count += 1
            filler_words.append(current_word.lower())

    # Also check the last word for being a filler word
    if transcription.words[-1]['word'].lower() in filler_words:
        filler_word_count += 1
